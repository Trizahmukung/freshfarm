    ...
    <div>
      ...
      <div class="row item-entry gx-2">
          ...
          <div class="form-group col-sm-1 col-md-1 col-lg-1">
              ...
              <input type="number" name="quantity" v-model="itemData.quantity" class="form-control line-input px-1" v-on:input="calculate" min="0" :max="(this.itemData.inventory_id)? itemData.maxQuantity : ''" step="1">
          ...
          <div class="form-group col-sm-1 col-md-1 col-lg-1">
              <button  class="btn btn-sm btn-warning w-100" @click.prevent="saveDocumentItem" :disabled="itemData.amount<=0 || (itemData.inventory_id && itemData.maxQuantity && itemData.quantity > itemData.maxQuantity)"><i class="fa fa-plus"></i></button>
          </div>
      </div>
      <div class="row">
        <div class="col-sm-12" v-if="itemData.inventory_id && itemData.maxQuantity && itemData.quantity > itemData.maxQuantity"><span class="text-danger">Remaining stock for item cannot fulfill the order. Remaining stock is {{itemData.maxQuantity}}</span></div>
      </div>
          ...
          methods: {
            ...
            getUnit: async function () {
              ...
              if(itemArray[0]=='INV'){
                  ...
                  this.itemData.maxQuantity=inventory.quantity;
              }
